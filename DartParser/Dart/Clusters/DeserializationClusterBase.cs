...
using DartParser.Dart.Clusters.ToCheck;
...
public class DeserializationClusterBase(bool isCanonical, bool isImmutable, ClassId cid, bool isRootUnit, SemVersion version)
{
    ...
    public static DeserializationClusterBase Create(Snapshot snapshot)
    {
        ...
        return cid switch
        {
            ...
            ClassId c
                when c.IsExternalTypedDataClassId()
                    => new ExternalTypedDataDeserializationCluster(isCanonical, isImmutable, cid, isRootUnit, version),
            ClassId.kPcDescriptorsCid
                or ClassId.kCodeSourceMapCid
                or ClassId.kCompressedStackMapsCid
                when !snapshot.UseCompressedPointers
                  && snapshot.Kind.IncludesCode()
                    => new RODataDeserializationCluster(isCanonical, isImmutable, cid, isRootUnit, version),
